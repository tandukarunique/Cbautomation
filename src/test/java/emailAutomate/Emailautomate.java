...
public class Emailautomate {
    ...
    // Constructor for normal mode (default)
    public Emailautomate() {
        this(false);
    }
    
    // Constructor with incognito option
    public Emailautomate(boolean incognitoMode) {
    ...
    private void generateTempEmail() {
        try {
            ...
            // Click the tick/alias button before copying email
            WebElement useAliasBtn = wait.until(ExpectedConditions.elementToBeClickable(
                By.id("use-alias")
            ));
            ...
            // Wait a moment for the email to update after clicking
            Thread.sleep(2000);

            // Now copy the email address
